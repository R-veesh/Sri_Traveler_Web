...
// const FieldValueServerTimestamp = FieldValue.serverTimestamp
const auth = firebase.auth()
const db = firebase.firestore()
const FieldValueServerTimestamp = firebase.firestore.FieldValue.serverTimestamp
...
auth.onAuthStateChanged((user) => {
  if (user) {
    ...
    // Load dashboard bookings if on dashboard page
    if (document.getElementById("dashboardBookings")) {
      loadDashboardBookings()
    }

    // Load booking stats if on dashboard page
...
if (tripForm) {
  tripForm.addEventListener("submit", async (e) => {
    ...
    const tripImageInput = document.getElementById("tripImagePath")
...
async function fetchTrips() {
  ...
  try {
    const querySnapshot = await db.collection("trips").where("guideId", "==", guideId).get()
    ...
    querySnapshot.forEach((doc) => {
      ...
      const tripElement = `
        <div class="trip-card">
          <img src="${trip.tripImagePath}" alt="${trip.tripName}" class="trip-image"/>
          <div class="trip-details">
            <h3>${trip.tripName}</h3>
            <p><strong>Place:</strong> ${trip.tripPlace}</p>
            ...
            <button class="edit-trip-btn" data-id="${doc.id}">Edit</button>
            <button class="delete-trip-btn" data-id="${doc.id}">Delete</button>
          </div>
        </div>
      `
      tripsContainer.innerHTML += tripElement
    })

    // Add event listeners for edit & delete buttons
    document.querySelectorAll(".edit-trip-btn").forEach((button) => button.addEventListener("click", editTrip))
    document
      .querySelectorAll(".delete-trip-btn")
      .forEach((button) => button.addEventListener("click", deleteTripFirestore))
  } catch (error) {
...
async function deleteTripFirestore(event) {
...
// Function to Edit a Trip (Opens an Edit Form)
function editTrip(event) {
  const tripId = event.target.dataset.id
  // Implement logic to open a modal or form to edit trip details
...
async function loadDashboardBookings() {
  const bookingsSection = document.getElementById("dashboardBookings")
  ...
  try {
    ...
    if (bookingsSnapshot.empty) {
      bookingsSection.innerHTML = '<p class="no-bookings">No bookings yet</p>'
      return
    }
    ...
    bookingsSnapshot.forEach((doc) => {
      ...
      const bookingDate = booking.createdAt ? booking.createdAt.toDate().toLocaleDateString() : "Unknown date"

      const bookingItem = document.createElement("div")
      bookingItem.className = "activity-item"
      ...
      bookingItem.innerHTML = `
        ...
        <div class="activity-details">
          ...
          <p>Booked by ${booking.userName || "Anonymous"}</p>
          <p>Status: <span class="status-${booking.status}">${booking.status}</span></p>
          <span class="activity-time">${bookingDate}</span>
        </div>
      `
    ...
    bookingsSection.innerHTML = `<p class="error-message">Error loading bookings: ${error.message}</p>`
  }
}

// Load booking stats for dashboard
async function loadBookingStats() {
  const pendingCountElement = document.getElementById("pendingBookingsCount")
  const confirmedCountElement = document.getElementById("confirmedBookingsCount")
  const totalBookingsElement = document.getElementById("totalBookingsCount")

  if (!pendingCountElement || !confirmedCountElement || !totalBookingsElement) return
  ...
  try {
    ...
    const totalSnapshot = await db.collection("bookings").where("guideId", "==", auth.currentUser.uid).get()

    pendingCountElement.textContent = pendingSnapshot.size
    confirmedCountElement.textContent = confirmedSnapshot.size
    totalBookingsElement.textContent = totalSnapshot.size
  } catch (error) {
